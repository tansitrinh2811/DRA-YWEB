...
import { addLiquidityExternal } from "../Utils/addLiq"; 
import { getLiquidityData } from "../Utils/checkLiq";
...
export const SwapTokenContextProvider = ({ children }) => {
  ...
  const fetchingData = async () => {
    try {
      ...
      console.log("userLiq", userLiquidity);
      ...
      userLiquidity.map(async (el, i) => {
        ...
        console.log("getAllLiq", getAllLiquidity);
      });
  ...
  const singleSwapToken = async ({ token1, token2, swapAmount }) => {
    ...
    try {
      ...
      console.log("soloswap", singleSwapToken)
      weth = await connectingWithIWTHToken();
      console.log("wethct", weth)
      dai = await connectingWithDAIToken();
      console.log("daict", dai)
